...
We'll need to wrap our react application with this `HashRouter` component in
...
We'll also want to import the components that we want the user to be able to
...
Now that we have all of our dependencies in `routes.js`, we can use an
...
Now that we have our new route setup in `./src/routes.js`, let's open up
...
Now that we have our `Student` route setup, let's open the `ClassList` component
